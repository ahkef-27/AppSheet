...
### ▼ メイン画面（マイタスク・ダッシュボード）
- ログインしたユーザー専用のタスクが一覧で表示されます。
![メイン画面](images/screenshot-main.png)

### ▼ タスク追加画面（入力フォーム）
- 直感的に操作できる入力フォームで、現場でも迷わずタスクを登録できます。
![タスク追加画面](images/screenshot-form.png)

### ▼ セキュリティ設定（USEREMAIL関数による絞り込み）
- 他のユーザーのデータを見せないための、スライス（Slices）の設定画面です。
![セキュリティ](images/screenshot-slice.png)

### ▼ 自動化通知（実際の受信メール）
- タスクが追加された瞬間、Botから担当者へ自動送信された実際のメール画面です。
![自動化通知](images/screenshot-mail.png)
